#version 300 es
// Cloud field from Tianxiu (Tyson) Zhou, based on mdb (ShaderToy Ndc3zl).
// Direct reference: user-supplied cloud_sea (1).html.
// See ./LICENSE and docs/references/up-in-the-cloud-sea/README.md.
// Adaptation: split background/foreground, anchor to the task railway,
// protect the root task from foreground fog. Cloud equations are retained.
precision highp float;
precision highp int;
uniform vec3 iResolution;
uniform float iTime;
uniform sampler2D iChannel0;
uniform int uForeground;
uniform float uAxisY;
uniform vec2 uCloudTravel; // independently integrated far / near cloud travel
uniform vec4 uCloudLook; // density, brightness, near motion blur, dynamic light
uniform vec4 uRootBounds;
uniform vec2 uCanvasSize;
// CSS pixel coordinates, supplied from the actual React Flow path/traffic pose.
uniform vec4 uRailway; // start x, end x, axis y, camera zoom
uniform vec4 uTrain;   // engine x, engine y, UV pixel scale, coach count
out vec4 fragColor;
float noise(vec2 x){
    vec2 f = fract(x);
...
#define layer(dh, v)  if (uv.y < h + midlevel - (dh) + (uCloudLook.x - 1.0) * .2 ) return vec4(v, 1.);
...
// Train, animated steam and bridge formulas copied from the user reference.
// Only their coordinates and train length are adapted to the mind-map axis.
vec3 railwayRaster(vec2 pixel, vec3 col, float t) {
    if (uRailway.w <= 0.0 || pixel.x < min(uRailway.x, uRailway.y)
        || pixel.x > max(uRailway.x, uRailway.y)) return col;
    vec2 uv;
    vec2 uv2;
    float k, h, dist;
    uv = vec2(.5 + (pixel.x - uTrain.x) / uTrain.z,
              .1 + (uTrain.y - pixel.y) / uTrain.z);
    if (uv.x > .45 - uTrain.w / 9.0 && uv.x < .51 && uv.y > .075 && uv.y < .55) {
    // choo choo
    k = 1.0;
    uv2 = fract(uv*9.0);
    float wagon = 1.0;
    ...
    // The fixed bridge is rendered as SVG so zoom never rasterizes its lines.
    return col;
}
...
vec3 vignettCorrection( vec3 col, vec2 uv )
{
    ...
    // varying lightness
    col1 *= (1.3 + uCloudLook.w * 0.4 * cos(0.5 * (iTime + 3.14))) * uCloudLook.y;
    return col1;
}
...
void main() {
    ...
    vec2 top = vec2(screen.x, 1.0 - screen.y);
    vec2 uv = gl_FragCoord.xy / iResolution.y;
    // In the reference, the rails sit at uv.y == .3.
    uv.y += uAxisY - .7;
    float t = sin(1.2 * iTime) + 4.0 * iTime;
    if (uForeground == 0) {
        // Cloud lighting can evolve; the solid railway keeps a constant color.
        vec3 clouds = vignettCorrection(background(uv, uCloudTravel.x).rgb, screen);
        fragColor = vec4(railwayRaster(top * uCanvasSize, clouds, t), 1.0);
        return;
    }
    // Leave the rails, cables and the tops of the piers unobscured. Only the
    // lower supports disappear into the flowing cloud sea.
    float clearDeck = uAxisY + max(14.0, 45.0 * uRailway.w) / uCanvasSize.y;
    if (uv.y >= .5 || top.y < clearDeck) {
        fragColor = vec4(0.0);
        return;
    }
    vec4 fog = vec4(0.0);
    // Five temporal samples from the reference soften the fast near clouds.
    if (uCloudLook.z <= 0.0) {
        fog = foreground(uv, uCloudTravel.y);
    } else {
        for (int i = 0; i < 5; i++) {
            fog += foreground(uv, uCloudTravel.y + uCloudLook.z * 4.0 * float(i) / 5.0 / 60.0) / 5.0;
        }
    }
    float rootDistance = max(max(uRootBounds.x - top.x, top.x - uRootBounds.z),
                             max(uRootBounds.y - top.y, top.y - uRootBounds.w));
    float mask = smoothstep(clearDeck, clearDeck + .045, top.y)
               * smoothstep(-.002, .008, rootDistance);
    fragColor = vec4(vignettCorrection(fog.rgb, screen), fog.a * mask);
}
